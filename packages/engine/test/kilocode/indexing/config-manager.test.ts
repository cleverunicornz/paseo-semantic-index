...
function createHostConfig(overrides: Partial<Parameters<typeof toIndexingConfigInput>[0]> = {}) {
  return IndexingConfig.parse({
    model: "test-embedding-model",
    dimension: 32,
    "openai-compatible": { baseUrl: "http://localhost:8001/v1" },
...
describe("CodeIndexConfigManager", () => {
  ...
  test("normalizes the host vector store to Qdrant", () => {
    expect(toIndexingConfigInput(createHostConfig()).vectorStoreProvider).toBe("qdrant")
  })

  test("preserves an explicit Qdrant override", () => {
    const input = toIndexingConfigInput(createHostConfig({ vectorStore: "qdrant" }))
  ...
  test("normalizes configured file extensions", () => {
    ...
    expect(toIndexingConfigInput(createHostConfig({ fileExtensions: ["PHP", ".JS"] })).fileExtensions).toEqual([
      ".js",
      ".php",
    ])
  ...
  test("validates file extension tokens", () => {
    expect(IndexingConfig.safeParse({ ...createHostConfig(), fileExtensions: ["php", " .JS "] }).success).toBe(true)
    expect(IndexingConfig.safeParse({ ...createHostConfig(), fileExtensions: [] }).success).toBe(false)
    expect(IndexingConfig.safeParse({ ...createHostConfig(), fileExtensions: ["*.js"] }).success).toBe(false)
    expect(IndexingConfig.safeParse({ ...createHostConfig(), fileExtensions: ["src/php"] }).success).toBe(false)
    expect(IndexingConfig.safeParse({ ...createHostConfig(), fileExtensions: [".d.ts"] }).success).toBe(false)
  })
